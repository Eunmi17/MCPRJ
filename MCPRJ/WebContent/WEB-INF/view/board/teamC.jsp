...
<script>
		...
		btn.onclick = function() {
			...
			$.ajax({
				...
				success : function(data) {
					if (!$('#name').val()) {
						$('#namec').html('<p style="color:red;">동호회명을 입력해주세요.</p>');
						$('#name').focus();
						nameCheck = false;
					}else if ($.trim(data) != 0) {
						$('#namec').html('<p style="color:red;">이미 존재하는 동호회 입니다.</p>');
						$('#name').focus();
						nameCheck = false;
					}else{
						$('#namec').html('<p style="color:green;">사용가능한 동호회명 입니다.</p>');
					nameCheck = true;
	...
	function doSubmit(f) {
		if(nameCheck == false) {
			$('#namec').html('<p style="color:red;">동호회명 중복체크를 해주세요.</p>');
			f.team_name.focus();
			return false;
		}
		if(f.join_form.checked == false) {
			$('#formc').html('<p style="color:red;">가입방식을 선택해주세요.</p>');
			f.join_form.focus();
			return false;
		}
	}
					...
					<div class="row">
						<div class="col-md-12">
							<div class="card">
								...
								<div class="card-body">
									<form method="post" onsubmit="return doSubmit(this)" action="/teamC_proc.do">
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													<label class="bmd-label-floating">동호회 명</label>&emsp;
													<button type="button" class="btn btn-primary" id="btn">중복 확인</button>
													<input type="text" name="team_name" class="form-control" id="name">
													<div id="namec"></div>
												</div>
											</div>
										</div>
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													...
													<textarea name="team_memo" class="form-control" rows="10" placeholder="환영합니다."></textarea>
												</div>
											</div>
										</div>
										<div class="row">
											<div class="col-md-12">
												<div class="form-group">
													...
													<label id="t"><input type="radio" class="togglebutton" name="join_form" value="A"> 자동 승인</label>
													&emsp;
													<label id="t"><input type="radio" class="togglebutton" name="join_form" value="S"> 리더 승인</label>
													<div id="formc"></div>
												</div>
											</div>
										</div>
